...
public class ConnectRs232 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//lbx add com13
		String[] eStrings = new String[entries.length+1];
		System.arraycopy(entries, 0, eStrings, 0, entries.length);
		eStrings[eStrings.length-1] = "Com13";
		entries = eStrings;
		...
		mConectButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					//lbx
					if (mPosPort==mPortList.size()-1) {
						cn.pda.serialport.SerialPort mhSerialPort = new cn.pda.serialport.SerialPort(13, Integer.parseInt(mBaudList.get(mPosBaud)), 0);
//						mhSerialPort.power_5Von();
//						mhSerialPort.rfid_poweron();
